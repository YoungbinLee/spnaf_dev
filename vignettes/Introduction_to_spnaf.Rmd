...
method = 't', R = 999, n.cores = 2) # you may use more cores to decrease running time
```
...
```{r, eval = TRUE}
# positive clusters at the significance level of 0.01
head(result[[1]][result[[1]]$pval_positive < 0.01,])
# positive clusters at the significance level of 0.01 in lines class
head(result[[2]][result[[2]]$pval_positive < 0.01,])
```

### Visualization of all flows and Significant Flows(<0.01) only
...
tm_shape(CA_polygon) +
  tm_polygons()+
  tm_shape(result[[2]][result[[2]]$pval_positive < 0.01,]) +
  tm_lines(col='pval_positive')
